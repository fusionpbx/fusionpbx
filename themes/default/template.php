...
//logged in, show the domains block
	if (strlen($_SESSION["username"]) > 0 && permission_exists("domain_select") && count($_SESSION['domains']) > 1) {
		...
		foreach($_SESSION['domains'] as $domain) {
			...
			echo "<a href='".escape($domain_path)."?domain_uuid=".escape($domain['domain_uuid'])."&domain_change=true' ".(($domain['domain_uuid'] == $_SESSION['domain_uuid']) ? "style='font-weight: bold;'" : null)."title='".escape($domain['domain_name'])." - ".escape($domain['domain_description'])."'> ".escape($domain['domain_name'])."</a>\n";
