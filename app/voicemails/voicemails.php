		...
		foreach ($voicemails as $row) {
			...
			if (permission_exists('voicemail_message_view') || permission_exists('voicemail_greeting_view')) {
				...
				if (permission_exists('voicemail_message_view')) {
					...
					echo "	<a href='voicemail_messages.php?id=".urlencode($row['voicemail_uuid'])."&back=".urlencode($_SERVER["REQUEST_URI"])."'>".$text['label-messages'].$tmp_voicemail_string."</a>\n";
				}
				echo "	</td>\n";
			}
			if (permission_exists('voicemail_edit')) {
