...
// Set variables from GET parameters
	...
	$order_by = preg_replace('#[^a-zA-Z0-9_\-]#', '', ($_GET['order_by'] ?? 'control_name'));
...
//process the http post data by action
	if (!empty($action) && !empty($conference_controls)) {
		...
		header('Location: conference_controls.php'.($query_string ? '?'.$query_string : ''));
		exit;
	}
...
//get the count
	...
	if (!empty($search)) {
		$sql .= "where (";
		$sql .= "	lower(control_name) like :search ";
		$sql .= "	or lower(control_description) like :search ";
		$sql .= ") ";
...
//show the content
	...
	if (permission_exists('conference_control_add')) {
		echo button::create(['type'=>'button','label'=>$text['button-add'],'icon'=>$settings->get('theme', 'button_icon_add'),'id'=>'btn_add','link'=>'conference_control_edit.php'.($query_string ? '?'.$query_string : '')]);
	}
	...
	echo th_order_by('control_name', $text['label-control_name'], $order_by, $order, null, null, $query_string);
	echo th_order_by('control_enabled', $text['label-control_enabled'], $order_by, $order, null, "class='center shrink'", $query_string);
	...
	if (!empty($conference_controls)) {
		...
		foreach ($conference_controls as $row) {
			if (permission_exists('conference_control_edit')) {
				$list_row_url = "conference_control_edit.php?id=".urlencode($row['conference_control_uuid']).($query_string ? '&'.$query_string : '');
			}
