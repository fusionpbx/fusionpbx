...
//get total extension count from the database
	$sql = "select ";
	$sql .= "(select count(*) from v_multinode_rabitMQ where domain_uuid = '".$_SESSION['domain_uuid']."' ".$sql_mod.") as num_rows ";
	if ($db_type == "pgsql") {
		$sql .= ",(select count(*) as count from v_multinode_rabitMQ ";
...
//show the content
	...
	if (is_array($multi_nodes)) {
		...
		foreach($multi_nodes as $row) {
			$tr_link = (permission_exists('extension_edit')) ? " href='multi_node_edit.php?id=".$row['multinode_rabitMQ_id']."'" : null;
			echo "<tr ".$tr_link.">\n";
			if (permission_exists('extension_delete')) {
				echo "	<td valign='top' class='".$row_style[$c]." tr_link_void' style='text-align: center; vertical-align: middle; padding: 0px;'>";
				echo "		<input type='checkbox' name='id[]' id='checkbox_".$row['multinode_rabitMQ_id']."' value='".$row['multinode_rabitMQ_id']."' onclick=\"if (!this.checked) { document.getElementById('chk_all').checked = false; }\">";
				echo "	</td>";
				$ext_ids[] = 'checkbox_'.$row['multinode_rabitMQ_id'];
			}
			echo "	<td valign='top' class='".$row_style[$c]."'>";
			if (permission_exists('extension_edit')) {
				echo "<a href='multi_node_edit.php?id=".$row['multinode_rabitMQ_id']."'>".$row['name']."</a>";
			}
			else {
				echo $row['name'];
			}
			...
			if (permission_exists('extension_edit')) {
				echo "<a href='multi_node_edit.php?id=".$row['multinode_rabitMQ_id']."' alt='".$text['button-edit']."'>$v_link_label_edit</a>";
			}
			if (permission_exists('extension_delete')) {
				echo "<a href='multi_node_delete.php?id[]=".$row['multinode_rabitMQ_id']."' alt='".$text['button-delete']."' onclick=\"return confirm('".$text['confirm-delete']."')\">$v_link_label_delete</a>";
			}
			echo "</td>\n";
			echo "</tr>\n";
			$c = ($c) ? 0 : 1;
