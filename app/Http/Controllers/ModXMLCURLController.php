    ...
    public function configuration(Request $request): string{
        ...
        switch ($request->input('key_value')){
            ...
            case 'format_cdr.conf':
                ...
                $xml->startElement('configuration');
                $xml->writeAttribute('name', $request->input('key_value'));
                $xml->writeAttribute('description', 'Multi Format CDR CURL logger');
                $xml->startElement('profiles');
                $xml->startElement('profile');
                ...
                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('format'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($format); $xml->endAttribute();
                $xml->endElement(); //param

                if (isset($url)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('url'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($url); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                if (isset($cred)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('cred'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($cred); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('retries'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($retries); $xml->endAttribute();
                $xml->endElement(); //param

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('delay'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($delay); $xml->endAttribute();
                $xml->endElement(); //param

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('log-http-and-disk'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($log_http_and_disk); $xml->endAttribute();
                $xml->endElement(); //param

                if (isset($log_dir)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('log-dir'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($log_dir); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('log-b-leg'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($log_b_leg); $xml->endAttribute();
                $xml->endElement(); //param

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('prefix-a-leg'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($prefix_a_leg); $xml->endAttribute();
                $xml->endElement(); //param

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('encode'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($encode); $xml->endAttribute();
                $xml->endElement(); //param

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('disable-100-continue'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($disable_100_continue); $xml->endAttribute();
                $xml->endElement(); //param

                if (isset($err_log_dir)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('err-log-dir'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($err_log_dir); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('auth-scheme'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($auth_scheme); $xml->endAttribute();
                $xml->endElement(); //param

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('enable-cacert-check'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($enable_cacert_check); $xml->endAttribute();
                $xml->endElement(); //param

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('enable-ssl-verifyhost'); $xml->endAttribute();    // TODO: check this in the freeswitch source code
                $xml->startAttribute('value'); $xml->text($enable_ssl_verify_host); $xml->endAttribute();
                $xml->endElement(); //param

                if (isset($ssl_cert_path)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('ssl-cert-path'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($ssl_cert_path); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                if (isset($ssl_key_path)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('ssl-key-path'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($ssl_key_path); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                if (isset($ssl_key_password)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('ssl-key-password'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($ssl_key_password); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                if (isset($ssl_cacert_file)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('ssl-cacert-file'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($ssl_cacert_file); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                if (isset($ssl_version)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('ssl-version'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($ssl_version); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                if (isset($cookie_file)){
                    $xml->startElement('param');
                    $xml->startAttribute('name'); $xml->text('cookie-file'); $xml->endAttribute();
                    $xml->startAttribute('value'); $xml->text($cookie_file); $xml->endAttribute();
                    $xml->endElement(); //param
                }

                $xml->startElement('param');
                $xml->startAttribute('name'); $xml->text('encode-values'); $xml->endAttribute();
                $xml->startAttribute('value'); $xml->text($encode_values); $xml->endAttribute();
                $xml->endElement(); //param
