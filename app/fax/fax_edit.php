...
//show the content
	...
	if (!permission_exists('fax_extension_delete')) {
		...
		if (!empty($_SESSION['ranges']['fax_range']['text'])) {
			echo "	<input class='formfld' type='text' name='fax_extension' maxlength='15' value=\"".escape($fax_extension ?? '')."\" required='required' placeholder='".$_SESSION['ranges']['fax_range']['text']."'>\n";
		}
		else {
		...
		echo "<br />\n";
		if (!empty($_SESSION['ranges']['fax_range']['text'])) {
			echo "".$text['description-extension']."<br />(ex: ".$_SESSION['ranges']['fax_range']['text'].")\n";
		}
		else {
