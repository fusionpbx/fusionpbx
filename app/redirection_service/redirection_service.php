...
//check permissions
	if (permission_exists('redirection_service_view')) {
		//access granted
	}
	else {
		echo "access denied";
...
//process the http post data by action
	if ($action != '' && is_array($devices) && @sizeof($devices) != 0) {
		...
		header('Location: redirection_service.php'.($search != '' ? '?search='.urlencode($search).'&fields='.urlencode($fields) : null));
		exit;
	}
...
//get total devices count from the database
	$sql = "select count(*) from v_redirection_service ";
