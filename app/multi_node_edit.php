...
//get the http values and set them as php variables
	if (count($_POST) > 0) {
		...
		if ($action == "add") {
			...
			$sql = "INSERT INTO v_multinode_rabitMQ 
					(multinode_rabitMQ_id, domain_uuid, name, node_priority, switch_name, hostname, virtualhost, username, password, 
		...
		//add or update the database
			...
			//assign the device to the extension 
				if ($action == "update") {
					...
					$domain_uuid = $_SESSION['domain_uuid'];
					$multinode_rabitMQ_id = $_REQUEST['multinode_rabitMQ_id'];
					
					$sql = "update v_multinode_rabitMQ set ";
					
					// $sql .= "multinode_rabitMQ_id   =   '".check_str($multinode_rabitMQ_id)."', ";
					...
					$sql .= "where multinode_rabitMQ_id = '".check_str($multinode_rabitMQ_id)."' and ";
					$sql .= "domain_uuid = '".check_str($domain_uuid)."'";
			...
			//show the action and redirect the user
				if ($action == "add") {
					...
					// header("Location: multi_node_edit.php?id=".$multinode_rabitMQ_id);
					header("Location: multi_node.php");
...
//pre-populate the form
	if (count($_GET) > 0 && $_POST["persistformvar"] != "true") {
		$multinode_rabitMQ_id = $_GET["id"];
		$sql = "select * from v_multinode_rabitMQ ";
		$sql .= "where multinode_rabitMQ_id = '".check_str($multinode_rabitMQ_id)."' ";
...
//begin the page content
	...
	if ($action == "update") {
		echo "		<input type='hidden' name='multinode_rabitMQ_id' value='".$multinode_rabitMQ_id."'>\n";
		echo "		<input type='hidden' name='id' id='id' value='".$multinode_rabitMQ_id."'>";
		if (!permission_exists('extension_domain')) {
