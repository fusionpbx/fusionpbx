...
//page title and description
	...
	echo "				</select>\n";
	echo "			</td>\n";
	echo "			<td style='vertical-align: top; padding-left: 15px;'>".escape($paging_controls_mini)."</td>\n";
	echo "		</tr>\n";
	echo "	</table>\n";
	echo "	</form>\n";
	echo "</td>\n";
	...
	//basic search of call detail records
		if (permission_exists('xml_cdr_search')) {
			...
			echo "<td width='".((if_group("admin") || if_group("superadmin") || if_group("cdr")) ? '24%' : '30%')."' style='vertical-align: top;'>\n";
				...
				echo "			<input type='text' class='formfld' style='".escape($style['caller_id_number'])."' name='caller_id_number' id='caller_id_number' value='".$caller_id_number."'>\n";
				echo "		</td>\n";
				echo "	</tr>\n";
				echo "	<tr>\n";
				echo "		<td class='vncell' valign='top' nowrap='nowrap'>\n";
		...
		foreach($result as $index => $row) {
			...
			//determine call result and appropriate icon
				...
				if ($theme_cdr_images_exist) {
					...
					if (strlen($row['direction']) > 0) {
						...
						$image_name .= ".png";
						echo "<img src='".PROJECT_PATH."/themes/".$_SESSION['domain']['template']['name']."/images/".escape($image_name)."' width='16' style='border: none; cursor: help;' title='".$text['label-'.escape($row['direction'])].": ".$text['label-'.escape($call_result)]. (escape($row['leg'])=='b'?'(b)':'') . "'>\n";
					}
				}
				else { echo "&nbsp;"; }
				echo "</td>\n";
			...
			//source
				...
				if (is_numeric($row['caller_id_number'])) {
					echo "		".escape(format_phone($row['caller_id_number'])).' ';
				}
				else {
			...
			//caller destination
				if (permission_exists('caller_destination')) {
					...
					if (is_numeric($row['caller_destination'])) {
						echo "		".escape(format_phone($row['caller_destination'])).' ';
					}
					else {
			...
			//tta (time to answer)
				echo "	<td valign='top' class='".$row_style[$c]."' style='text-align: right;'>".(($row['tta'] > 0) ? escape($row['tta'])."s" : "&nbsp;")."</td>\n";
			//duration
			...
			//control icons
				if (permission_exists('xml_cdr_details')) {
					...
					if (permission_exists('xml_cdr_delete')) {
						echo 	"<a href='xml_cdr_delete.php?id[]=".escape($row['uuid'])."&rec[]=".(($record_path != '') ? base64_encode($record_path.'/'.$record_name) : null)."' alt='".$text['button-delete']."' onclick=\"return confirm('".$text['confirm-delete']."')\">".$v_link_label_delete."</a>";
					}
					echo "	</td>\n";
				}
			echo "</tr>\n";
