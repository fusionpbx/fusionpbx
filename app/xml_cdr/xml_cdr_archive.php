...
/*				if (permission_exists('xml_cdr_delete')) {
					...
					echo "		<input type='checkbox' name='id[".$index."]' id='checkbox_".escape($row['uuid'])."' value='".escape($row['uuid'])."' onclick=\"if (this.checked) { document.getElementById('recording_".escape($row['uuid'])."').value='".base64_encode($record_path.'/'.$record_name)."' } else { document.getElementById('recording_".escape($row['uuid'])."').value=''; document.getElementById('chk_all').checked = false; }\">";
					echo "		<input type='hidden' name='rec[".$index."]' id='recording_".escape($row['uuid'])."'>";
					echo "	</td>";
...
*/			//determine call result and appropriate icon
				...
				if ($theme_cdr_images_exist) {
					...
					if (strlen($row['direction']) > 0) {
						...
						$image_name .= ".png";
						echo "<img src='".PROJECT_PATH."/themes/".$_SESSION['domain']['template']['name']."/images/".escape($image_name)."' width='16' style='border: none; cursor: help;' title='".$text['label-'.escape($row['direction'])].": ".$text['label-'.$call_result]. ($row['leg']=='b'?'(b)':'') . "'>\n";
					}
				}
				else { echo "&nbsp;"; }
				echo "</td>\n";
			...
			//source
				...
				if (is_numeric($row['caller_id_number'])) {
					echo "		".format_phone(escape($row['caller_id_number'])).' ';
				}
				else {
			...
			//destination
				echo "	<td valign='top' class='".$row_style[$c]." tr_link_void' nowrap='nowrap'>";
				echo "		<a href=\"javascript:void(0)\" onclick=\"send_cmd('".PROJECT_PATH."/app/click_to_call/click_to_call.php?src_cid_name=".urlencode($row['destination_number'])."&src_cid_number=".urlencode(escape($row['destination_number']))."&dest_cid_name=".urlencode($_SESSION['user']['extension'][0]['outbound_caller_id_name'])."&dest_cid_number=".urlencode($_SESSION['user']['extension'][0]['outbound_caller_id_number'])."&src=".urlencode($_SESSION['user']['extension'][0]['user'])."&dest=".urlencode(escape($row['destination_number']))."&rec=false&ringback=us-ring&auto_answer=true');\">\n";
			...
			//billing
				if (file_exists($_SERVER["PROJECT_ROOT"]."/app/billing/app_config.php")){
					...
					echo "	<td valign='top' class='".$row_style[$c]."'>".number_format($price,6)." escape($billing_currency)</td>\n";
...
//					if (permission_exists('xml_cdr_delete')) {
//						echo 	"<a href='xml_cdr_delete.php?archive_request=true&id[]=".escape($row['uuid'])."&rec[]=".(($record_path != '') ? base64_encode($record_path.'/'.$record_name) : null)."' alt='".$text['button-delete']."' onclick=\"return confirm('".$text['confirm-delete']."')\">".$v_link_label_delete."</a>";
//					}
					echo "	</td>\n";
				}
			echo "</tr>\n";
