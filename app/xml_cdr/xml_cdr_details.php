...
//channel data loop
	...
	if (is_array($array["channel_data"])) foreach($array["channel_data"] as $key => $value) {
		...
		echo "	<td valign='top' align='left' class='".$row_style[$c]."'>".escape(wordwrap($value),75,"<br />\n", TRUE)."&nbsp;</td>\n";
		echo "</tr>\n";
		$c = $c ? 0 : 1;
	}
	echo "</table>";
...
//app_log
	...
	if (is_array($array["app_log"]["application"])) {
		foreach ($array["app_log"]["application"] as $row) {
			...
			echo "	<td valign='top' align='left' class='".$row_style[$c]."'>".escape(wordwrap($app_data),75,"<br />\n", true)."&nbsp;</td>\n";
			echo "</tr>\n";
			$c = $c ? 0 : 1;
		}
	}
