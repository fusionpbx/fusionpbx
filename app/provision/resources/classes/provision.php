...
class provision {
	...
	public function __construct(array $params = []) {
		...
		//device_address will always be empty in the constructor
...
//				}
		...
		if (PHP_OS == "Linux") {
			if (file_exists('/usr/share/fusionpbx/templates/provision')) {
				...
				$template_dir = $project_root . '/resources/templates/provision';
			}
	...
	private function device_exists($device_address) {
		...
		self::normalize_device_address($device_address);
	...
	public function render() {
		...
		$device_address = self::normalize_device_address($this->device_address);

		//remove ../ and slashes in the file name
		$search = array('..', '/', '\\', '/./', '//');
		$file = str_replace($search, "", $this->file);

		//domain uuid must be valid
		...
		if (empty($domain_name)) {
			$domain_name = domains::get_name_by_uuid($this->database, $domain_uuid);
		}
...
#			$templates['yealink SIP-CP860'] = 'yealink/cp960';
			...
			if ($this->settings->get('provision','auto_insert_enabled','false') === "true") {
				...
				$p = new permissions($this->database, $domain_uuid);
		...
		if (is_uuid($device_uuid) && is_uuid($device_profile_uuid)) {
			...
			if (!empty($device_profile_settings)) {
		...
		$view->cache_dir = $this->settings->get('server','temp', sys_get_temp_dir());
		$view->init();
		...
		if (is_uuid($device_uuid)) {
			...
			if (!empty($database_device_lines)) {
				foreach ($database_device_lines as $row) {
					$id = $row['line_number'];
					$device_lines[$id] = $row;
				}
			}
		...
		foreach ($types as $type) {
			if (!empty($device_keys[$type]) && is_array($device_keys[$type])) {
				foreach ($device_keys[$type] as $row) {
					...
					$device_key_value = str_replace("\${domain_name}", $domain_name, $row['device_key_value'] ?? ''); //1
					$device_key_extension = str_replace("\${domain_name}", $domain_name, $row['device_key_extension'] ?? '');
					$device_key_label = str_replace("\${domain_name}", $domain_name, $row['device_key_label'] ?? ''); //label
					$device_key_icon = str_replace("\${domain_name}", $domain_name, $row['device_key_icon'] ?? ''); //icon

					//grandstream modes are different based on the category
					if ($device_vendor == "grandstream") {
						if ($device_key_category == "line") {
							switch ($device_key_type) {
								case "line": $device_key_type = "0";
									break;
								case "shared line": $device_key_type = "1";
									break;
								case "speed dial": $device_key_type = "10";
									break;
								case "blf": $device_key_type = "11";
									break;
								case "presence watcher": $device_key_type = "12";
									break;
								case "eventlist blf": $device_key_type = "13";
									break;
								case "speed dial active": $device_key_type = "14";
									break;
								case "dial dtmf": $device_key_type = "15";
									break;
								case "voicemail": $device_key_type = "16";
									break;
								case "call return": $device_key_type = "17";
									break;
								case "transfer": $device_key_type = "18";
									break;
								case "call park": $device_key_type = "19";
									break;
								case "monitored call park": $device_key_type = "26";
									break;
								case "intercom": $device_key_type = "20";
									break;
								case "ldap search": $device_key_type = "21";
									break;
								case "phonebook": $device_key_type = "30";
									break;
							}
						}
						if ($device_key_category == "memory" || $device_key_category == "expansion") {
							switch ($device_key_type) {
								case "speed dial": $device_key_type = "0";
									break;
								case "blf": $device_key_type = "1";
									break;
								case "presence watcher": $device_key_type = "2";
									break;
								case "eventlist blf": $device_key_type = "3";
									break;
								case "speed dial active": $device_key_type = "4";
									break;
								case "dial dtmf": $device_key_type = "5";
									break;
								case "voicemail": $device_key_type = "6";
									break;
								case "call return": $device_key_type = "7";
									break;
								case "transfer": $device_key_type = "8";
									break;
								case "call park": $device_key_type = "9";
									break;
								case "monitored call park": $device_key_type = "16";
									break;
								case "intercom": $device_key_type = "10";
									break;
								case "ldap search": $device_key_type = "11";
									break;
							}
						}
					}
		...
		$view->assign("project_path", dirname(__DIR__, 4));
...
//		$view->assign("password", $password ?? '');
		...
		if (!empty($time_zone_name)) {
			...
			$time_zone_offset_minutes = number_pad(($time_zone_offset_raw - $time_zone_offset_hours) * 60, 2);
		...
		if ($this->settings->get('provision','debug','false') === 'true') {
			...
			$tmp_string .= $template_dir . "\n";
			$tmp_string .= $device_template . "\n";
			$tmp_string .= $file . "\n";
			$tmp_string .= $file_contents . "\n";
	...
	function write() {
		...
		if (!empty($result)) {
			foreach ($result as &$row) {
				...
				//clear the file_exists stat cache
				clearstatcache();
	...
	/**
	 * Normalizes the text by removing any characters that are not in the ranges a-f, A-F, or 0-9, and then converts the remaining characters to lowercase.
	 * @param string $device_address Device MAC address or UUID
	 * @return string Normalized address or empty string
	 */
	public static function normalize_device_address(string $device_address): string {
		//guard against empty value
		if (empty($device_address)) {
			return '';
		}
		//replace invalid characters with an empty value to remove them
		return strtolower(preg_replace('#[^a-fA-F0-9./]#', '', $device_address));
	}
