...
//system status
	...
	if (PHP_OS == 'FreeBSD' || PHP_OS == 'Linux') {
		...
		if (!empty($percent_disk_usage)) {
			...
			echo "	<div style='display: flex; flex-wrap: wrap; justify-content: center; padding-bottom: 20px; background-color: ".$dashboard_number_background_color.";' ".($row['dashboard_details_state'] == "disabled" ?: "onclick=\"$('#hud_system_status_details').slideToggle('fast');\"").">\n";
