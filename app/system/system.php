...
//system information
	...
	if (permission_exists('system_view_info')) {
		...
		$fpbx_git_path = normalize_path_to_os($_SERVER["DOCUMENT_ROOT"]."/.git");
		$fpbx_git_branch = shell_exec('git --git-dir='.$fpbx_git_path.' name-rev --name-only HEAD');
		rtrim($fpbx_git_branch);
		$fpbx_git_commit = shell_exec('git --git-dir='.$fpbx_git_path.' rev-parse HEAD');
		rtrim($fpbx_git_commit);
		$fpbx_git_origin = shell_exec('git --git-dir='.$fpbx_git_path.' config --get remote.origin.url');
		rtrim($fpbx_git_commit);
		echo "<tr>\n";
		echo "	<td width='20%' class=\"vncell\" style='text-align: left;'>\n";
		echo "		".$text['label-fpbx_git_info']."\n";
		echo "	</td>\n";
		echo "	<td class=\"row_style1\">\n";
		echo "		".$text['label-git_branch']." ".$fpbx_git_branch."<br>\n";
		echo "		".$text['label-git_commit']." ".$fpbx_git_commit."<br>\n";
		echo "		".$text['label-git_origin']." ".$fpbx_git_origin."<br>\n";
		echo "	</td>\n";
		echo "</tr>\n";
