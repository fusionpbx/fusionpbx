...
class tftpservice extends TFTPServer
{
  ...
  private function log($client, $level, $message) {   
	...
	{
		try {
			if($this->_debug) $this->log($client,"D", "Requested: ".$filepath);
			...
			if($this->_debug) $this->log($client,"D", "Sending File");
		}
	...
	{	
		...
		if($this->_debug) $this->log($client,"D", "Generating File");
		...
		if($data === false) 
		{
			$this->log($client,"W", "Generating File Failed ".$path);
			return false;
		}
		else
		{
	...
	public function retrieve_file($client, $path){
		...
		if($this->_debug) $this->log($client,"D", "Retrieve File");	
