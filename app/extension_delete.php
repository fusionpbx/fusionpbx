		...
		foreach($extension_uuids as $extension_uuid) {
			...
			if ($extension_uuid != '') {
				...
				//delete the extension
					...
					$sql = "delete from v_hotel_rooms ";
					$sql .= "where domain_uuid = '".$_SESSION['domain_uuid']."' ";
					$sql .= "and extension_number = '".$extension."' ";
					$prep_statement = $db->prepare(check_sql($sql));
					$prep_statement->execute();
					unset($prep_statement, $sql);

					//delete the Hotel Room Dashboard
					$sql = "delete from v_hotel_room_dashboard ";
					$sql .= "where domain_uuid = '".$_SESSION['domain_uuid']."' ";
					$sql .= "and room_number = '".$extension."' ";
					$prep_statement = $db->prepare(check_sql($sql));
					$prep_statement->execute();
					unset($prep_statement, $sql);
