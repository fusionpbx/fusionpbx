...
//build hud block html
	...
	//system/domain counts
		if (is_array($selected_blocks) && in_array(array('counts','system_counts'), $selected_blocks)) {
			if (in_array('counts', $selected_blocks)) {
				$scopes[] = 'domain';
			}
			if (permission_exists('domain_view') && in_array('system_counts', $selected_blocks)) {
				$scopes[] = 'system';
			}
