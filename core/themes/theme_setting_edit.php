...
//get http post variables and set them to php variables
	if (!empty($_POST)) {
		...
		$theme_setting_type = $_POST["theme_setting_type"] ?? null;
...
//process the data and save it to the database
	if (!empty($_POST) && empty($_POST["persistformvar"])) {
		...
		//check for all required data
			...
			if (empty($theme_setting_type)) { $msg .= $text['message-required']." ".$text['label-theme_setting_type']."<br>\n"; }
		...
		//prepare the array
			...
			$array['theme_settings'][0]['theme_setting_type'] = $theme_setting_type;
...
//pre-populate the form
	if (is_array($_GET) && empty($_POST["persistformvar"])) {
		...
		if (is_array($row) && @sizeof($row) != 0) {
			...
			$theme_setting_type = $row["theme_setting_type"];
...
//set the defaults
	$theme_setting_type = !empty($theme_setting_type) ? $theme_setting_type : 'text';
...
//show the content
	...
	echo "	".$text['label-theme_setting_name']."\n";
	echo "</td>\n";
	echo "<td class='vtable' style='position: relative;' align='left'>\n";
	echo "	<input class='formfld' type='text' name='theme_setting_name' maxlength='255' value='".escape($theme_setting_name)."'>\n";
	echo "<br />\n";
	...
	echo "	<select class='formfld' id='theme_setting_type' name='theme_setting_type' required='required'>\n";
	echo "		<option value=''></option>\n";
	if (!empty($setting_types)) {
		foreach ($setting_types as $setting_type) {
			echo "	<option value='".strtolower($setting_type)."' ".($theme_setting_type == strtolower($setting_type) ? "selected='selected'" : null).">".$setting_type."</option>\n";
		}
	}
	echo "	</select>\n";
	...
	echo "<tr ".(($theme_setting_type != 'array') ? "style='display: none;'" : null).">\n";
