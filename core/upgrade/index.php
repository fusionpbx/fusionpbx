...
//adjust color and initialize step counter
	$step = 1;
	$step_color = isset($settings->get('theme', 'upgrade_step_color')) ? $settings->get('theme', 'upgrade_step_color') : color_adjust((!empty($settings->get('theme', 'form_table_label_background_color')) ? $settings->get('theme', 'form_table_label_background_color') : '#e5e9f0'), -0.1);
...
//show the content
	...
	if (permission_exists("upgrade_source") && !is_dir("/usr/share/examples/fusionpbx") && is_writeable($_SERVER["PROJECT_ROOT"]."/.git")) {
		...
		echo "		".(isset($settings->get('theme', 'title')) ? $settings->get('theme', 'title') : 'FusionPBX')."\n";
		echo "	</td>\n";
		echo "	<td width='70%' class='vtable' style='height: 50px; cursor: pointer;'>\n";
		...
		if ($branch_return_value == 0 && $commit_return_value == 0) {
			...
			echo "&nbsp;&nbsp;<button type='button' class='btn btn-link btn-xs' onclick=\"event.stopPropagation(); source_preview('core','".(isset($settings->get('theme', 'title')) ? $settings->get('theme', 'title') : 'FusionPBX')."');\">".$text['button-preview']."</button>\n";
		}
		echo "	</td>\n";
		echo "</tr>\n";
		echo "</table>\n";
