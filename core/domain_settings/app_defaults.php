		...
		//update any domains set to legacy languages
			...
			foreach ($language->legacy_map as $language_code => $legacy_code) {
				if(strlen($legacy_code) == 5)
					continue;
				$sql = "update v_domain_settings set domain_setting_value = '$language_code' where domain_setting_value = '$legacy_code' and domain_setting_name = 'code' and domain_setting_dubcategory = 'language' and domain_setting_category = 'domain'";
				$db->exec(check_sql($sql));
				unset($sql);
			}
