...
function save_var_xml() {
	if (is_array($_SESSION['switch']['conf'])) {
		...
		//open the vars.xml file
			$filename = $_SESSION['switch']['conf']['dir']."/vars.xml";
			$fout = fopen($filename, "w");
			if(!$fout){
				messages::add("Failed to open '$filename' for writing", 'negative');
				return;
			}
		...
		//build the xml
			...
			$result = $prep_statement->fetchAll(PDO::FETCH_ASSOC);
			$xml = "<include>\n";
			$xml .= "<!-- This file is auto generated by the FusionPBX system, do not edit directly -->\n";
			foreach ($result as &$row) {
				if ($row['var_cat'] != 'Provision') {
					...
					if (strlen($row['var_hostname']) == 0 or $row['var_hostname'] == $hostname) {
						if (strlen($row["var_description"]) > 0) {
							$xml .= "\t<!-- ".$row['var_name']." :-\n\t\t".preg_replace('/\n/',"\n\t\t", base64_decode($row['var_description']))."\n\t-->\n";
						}
						...
						//prepare Agent XML string
							...
							foreach ($result as &$row) {
								...
								//if you change this variable also change app/call_center/call_center_agent_edit.php
									...
									if(strstr($agent_contact, '}') === FALSE) { //not found
										...
										if(stristr($agent_contact, 'sofia/gateway') === FALSE) { //not found
