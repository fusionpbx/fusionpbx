			...
			<div class="card card-primary mb-4">
				...
				<form method="POST" action="{{ $menu->exists ? route('menu.update', $menu->menu_uuid) : route('menu.store') }}">
					...
					<div class="card-footer">
						...
						@if($menu->exists)
						<a href="{{ route('menu.destroy', $menu->menu_uuid) }}"><button type="button" class="btn btn-danger">Delete</button></a>
						<a href="{{ route('menu_item.create', $menu->menu_uuid) }}"><button type="button" class="btn btn-success">Add Item</button></a>
						@endif
					</div>
				</form>
			</div>
