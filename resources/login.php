...
//set the current domain_uuid and domain_name
	$domain_uuid = $_SESSION['domain_uuid'] ?? '';
	$domain_name = $_SESSION['domain_name'] ?? '';

//use the HTTP_HOST to find domain_uuid and domain_name
...
//use the HTTP_HOST to find domain_uuid and domain_name
	if (empty($domain_uuid) || empty($domain_name)) {
		echo "Password Reset: domain_uuid or domain_name not found\n";
		exit;
	}
...
//set the password reset domain
	$password_reset_domain = $settings->get('login', 'password_reset_domain', $domain_name);
...
//send an email with the password reset link
	if (!empty($action) && $action == 'request' && !empty($_REQUEST['email'])) {
		if (valid_email($_REQUEST['email'])) {
			...
			if (is_array($result) && @sizeof($result) != 0) {
				if (!empty($result['username'])) {
					...
					//generate the reset link email and body variables
					...
					//get the user language code, if exists
...
//reset the password
	if ($_SESSION['valid_reset'] && !empty($_SESSION['valid_username']) && !empty($_REQUEST['password_new'])  && !empty($_REQUEST['password_repeat'])) {
		...
		if ($password_new === $password_repeat && check_password_strength($password_new, $text, 'user')) {
			...
			remember_me::delete_user_tokens($_SESSION['valid_username']);
