<?php
/**
 * Active Calls API - Hangup Call
 * POST /api/v1/active-calls/hangup.php
 *
...
// Execute hangup command using uuid_kill
$hangup_result = event_socket::api("uuid_kill {$call_uuid}");

// Check if hangup was successful
if ($hangup_result === false) {
    api_error('HANGUP_FAILED', 'Failed to execute hangup command', null, 500);
}

if (stripos($hangup_result, '-ERR') !== false) {
    api_error('HANGUP_FAILED', trim($hangup_result), null, 500);
}

api_success([
    'call_uuid' => $call_uuid,
    'result' => trim($hangup_result)
